...
function makeMacAppBundle(appName, binSource, targetBinName, platformName) {
...
<dict>
  ...
  // Copy pristine binary from bin/ into Contents/MacOS/
  const targetExec = path.join(appPath, 'Contents/MacOS', targetBinName);
  fs.copyFileSync(binSource, targetExec);
  try {
    execSync(`chmod +x "${targetExec}"`);
  } catch {}
  ...
  // Also copy neutralino.config.json if present
  ...
  const appSize = (fs.statSync(targetExec).size / (1024 * 1024)).toFixed(2);
...
binaryMap.forEach(({ raw, target, platform, binSource, appName }) => {
  ...
  if (fs.existsSync(rawPath)) {
    ...
    if (binSource && appName) {
      makeMacAppBundle(appName, binSource, target, platform);
    }
  } else {
...
if (fs.existsSync('bin/neutralino-mac_universal')) {
  makeMacAppBundle('RestStudio', 'bin/neutralino-mac_universal', 'RestStudio', 'macOS (Default App Bundle)');
}
