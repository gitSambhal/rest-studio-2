...
interface QuickNewRequestModalProps {
  ...
  onCreateScratchpadRequest?: (method: HTTPMethod, name: string, url?: string, extraRequestProps?: Partial<RestRequest>) => void;
}
...
export const QuickNewRequestModal: React.FC<QuickNewRequestModalProps> = ({
  ...
  const defaultTargetFileId = activeFileId || (files.length > 0 ? files[0].id : 'SCRATCHPAD');
  ...
  const handleApplySmartPaste = () => {
    if (!detectedPaste || !detectedPaste.requests || detectedPaste.requests.length === 0) return;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (targetFileId === 'SCRATCHPAD') {
      if (onCreateScratchpadRequest) {
        onCreateScratchpadRequest(selectedMethod, finalName, finalUrl, extraProps || undefined);
      }
    } else if (targetFileId === 'NEW_FILE') {
    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
            ...
            <div>
              ...
              <p className={`text-[11px] mt-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                Supports 3-level environment variables (Global, Project, File) like{' '}
                <code className="text-emerald-500 font-mono">{'{{baseUrl}}'}</code> or paste cURL, Postman JSON, REST
                snippet, or OpenAPI spec.
              </p>

              {/* Smart Paste Detection Banner */}
              {detectedPaste.type !== 'unknown' &&
                detectedPaste.type !== 'url' &&
                (detectedPaste.requests || []).length > 0 && (
                  <div
                    className={`mt-2.5 p-3 rounded-xl border text-xs flex items-center justify-between animate-in fade-in ${
                      isDarkMode
              ...
              {/\{\{[^}]+\}\}/.test(requestUrl) && (
                <div
                  className={`mt-2.5 p-3 rounded-xl border text-xs space-y-2.5 ${
                ...
                >
                  <div className="flex items-center justify-between text-[11px] font-semibold">
                    ...
                    >
                      {resolution.missingVars.length === 0
                        ? '✓ Resolved'
                        : `⚠️ ${resolution.missingVars.length} Unresolved`}
                    </span>
                  </div>
                      ...
                      >
                        ...
                        <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block z-50 w-64 p-2.5 bg-slate-900 border border-rose-500/40 rounded-xl shadow-2xl text-slate-100 font-sans text-xs space-y-1 animate-in fade-in zoom-in-95 pointer-events-none">
                          ...
                          <p className="text-[11px] text-slate-300 pt-1">
                            ...
                            found in active environment or target file.
                          </p>
                        </div>
                      </div>
                    ))}
            ...
            {targetFileId === 'NEW_FILE' && (
              <div
                className={`p-3 border rounded-xl space-y-2 animate-in fade-in ${
          ...
          >
            <span
              className={`text-[11px] flex items-center space-x-1 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}
            >
              <span>Press</span>
              <kbd
